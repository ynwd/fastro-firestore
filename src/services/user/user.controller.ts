...
export class UserController {
  ...
  async getAll (request: FastifyRequest, reply: FastifyReply): Promise<User[]> {
    const users: any = await this.userService.getAllUser()
  ...
  @Get({ url: '/1' })
  async getAll1 (request: FastifyRequest, reply: FastifyReply): Promise<User[]> {
  ...
  @Post({ url: '/1' })
  async register1 (request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const payload: any = request.body
  ...
  @Get({ url: '/2' })
  async getAll2 (request: FastifyRequest, reply: FastifyReply): Promise<User[]> {
    const users = await this.userService1.getAllUser()
    if (users.length === 0) reply.error(new Error('User not found'))
    return users
  }
  ...
  @Post({ url: '/2' })
  async register2 (request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const payload: any = request.body
    const user = await this.userService1.addUser(payload)
    reply.ok(user)
  }
